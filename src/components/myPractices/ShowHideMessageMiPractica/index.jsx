
import { Button, Col, Container, Row, Toast } from "react-bootstrap"
import { useShow } from "../../hook/useShow"

export const ShowHideMessageMiPractica = () => {

const {show, handleShowMessage} = useShow()

return(

<Container>
    <Row className="mt-5">
        <Col xs={12} lg={{span:6,offset:3}} className={'text-center'}>
        <Button className="mb-2" variant={show ? 'secondary' : 'success'} onClick={handleShowMessage}>
         {show ? 'Ocultar' : 'Mostrar'} mensaje
        </Button>
        <Toast show={show} onClose={handleShowMessage} className={'m-auto'}>
          <Toast.Header>
            <img
              src="holder.js/20x20?text=%20"
              className="rounded me-2"
              alt=""
            />
            <strong className="me-auto">React</strong>
            <small>Java Script</small>
          </Toast.Header>
          <Toast.Body>Aprendiendo React !</Toast.Body>
        </Toast>
        </Col>
    </Row>
</Container>
)

}
